...
class ParseHelper {
    ...
    static func likesForPosts(post: Post, completionBlock: PFQueryArrayResultBlock) {
        let query = PFQuery(className: ParseLikeClass)
        query.whereKey(ParseLikeToPost, equalTo: post)
    ...
    :param: user The user whose folowees you want to retrieve
    ...
    /** Establishes a relationship between users two users.
    ...
    /** Fetch users whose usernames match the provided search terms
    ...
    static func searchUsers(searchText: String, completionBlock: PFQueryArrayResultBlock) -> PFQuery {
        /* NOTE: We are using a Regex to allow for a case insensitive compare of usernames.
         Regex can be slow on large datsets. For large amounts of data it's better to store
         lowercased usernames in a separate column and perform a regular string compare. */
