...
class FriendSearchViewController: UIViewController {
   ...
   // stores all the users that match the current search query
    ...
    /* This is a local cache. It stores all of the users this user is following.
    It is used to updat the UI immediately upon user interaction, instead of 
    having to wait for a server response */
    
    var followingUsers: [PFUser]? {
        didSet {
            /* The list of following users may be fetched after the tableView has
            displayed cells. In this case, we reload the data to reflect "following" status. */
